...
# bad plugin sets (see docs/DEVELOPMENT.md). Keep this script and
...
#   name  kind  width  height  duration_ns  fps_num  fps_den  vfr  long  description
#
...
catalogue=$(
    cat <<'EOF'
bars_1080p_h264.mp4	video	1920	1080	5000000000	25	1	false	false	1080p SMPTE colour bars, H.264, timecode burn-in
bars_2160p_h264.mp4	video	3840	2160	5000000000	25	1	false	false	4K SMPTE colour bars, H.264, timecode burn-in
dropframe_2997_h264.mp4	video	1920	1080	10010000000	30000	1001	false	false	29.97 drop-frame clip with drop-frame timecode burn-in
vfr_60_30.mkv	video	1280	720	6000000000	60	1	true	false	Variable-frame-rate clip: 3 s at 30 fps then 3 s at 60 fps
longgop_720p_10min.mp4	video	1280	720	600000000000	25	1	false	true	10-minute long-GOP H.264 clip (250-frame GOP, B-frames)
tone_48k_stereo.wav	audio	0	0	5000000000	0	1	false	false	Audio only: 5 s 440 Hz sine, 48 kHz stereo, 16-bit WAV
tone_48k_stereo.flac	audio	0	0	5000000000	0	1	false	false	Audio only: 5 s 440 Hz sine, 48 kHz stereo, FLAC
EOF
)

if [ "$list_only" -eq 1 ]; then
    printf '%-26s %-6s %-9s %-4s %s\n' NAME KIND DURATION VFR DESCRIPTION
    while IFS=$'\t' read -r name kind _w _h dur_ns _fn _fd vfr long desc; do
        [ -n "$name" ] || continue
        suffix=""
        [ "$long" = "true" ] && suffix=" (--long only)"
        printf '%-26s %-6s %6s ms %-4s %s%s\n' \
            "$name" "$kind" "$((dur_ns / 1000000))" "$vfr" "$desc" "$suffix"
    done <<<"$catalogue"
    exit 0
fi
...
gen_audio() {
    ...
    *)
    ...
    run audiotestsrc wave=sine freq=440 samplesperbuffer=4800 num-buffers=50 \
        ...
        ! audioconvert ! "$encoder" ! filesink location="$out_dir/$1"
}

# ---------------------------------------------------------------- generate --
manifest_entries=""
while IFS=$'\t' read -r name kind width height dur_ns fps_n fps_d vfr long desc; do
    [ -n "$name" ] || continue
    generated=true
