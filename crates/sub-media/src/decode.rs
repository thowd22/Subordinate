...
fn wait_for_audio_format(
    ...
    loop {
        ...
        if let Some(err) = pipeline_error(pipeline) {
            // A file with no audio at all often fails as a pipeline error
            // instead of simply exposing no pad; that is not an error here.
            if no_more_pads.load(Ordering::SeqCst) && !saw_audio.load(Ordering::SeqCst) {
                return Ok(None);
            }
            return Err(err);
        }
